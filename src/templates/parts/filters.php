...
global $smap;
if ($smap['page'] == 'browser' || !empty($smap['entity'])){
	...
	<div class="header-filters"<?php if (has_filter()) echo ' style="display: block"'; ?>>
		<ul>
			<?php if (empty($smap['entity'])){ ?>
				<li id="top-filter-location" class="menu<?php if (!empty($smap['filters']['etype'])) echo ' top-filter-active'; ?>"><span class="menu-button">Location<?php
				...
				?>
				<li id="top-filter-entity-type" class="menu<?php if (!empty($smap['filters']['etype'])) echo ' top-filter-active'; ?>"><span class="menu-button">Entity type<?php
						...
						<div class="menu-menu">
							<ul class="menu-inner">
								...
								foreach ($types as $type => $c){ 
									...
									if ($type == 'company'){
										$subtypes = query('SELECT country, subtype FROM entities WHERE type = "company" AND subtype IS NOT NULL GROUP BY country, subtype ORDER BY country ASC, subtype ASC');
